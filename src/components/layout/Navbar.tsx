    ...
    <>
      ...
      <div className="bg-gradient-to-r from-primary-dark via-primary to-accent text-white py-2 hidden md:block relative overflow-hidden">
        ...
        <div className="container-custom flex justify-between items-center text-xs font-semibold tracking-wider relative z-10">
          ...
          <div className="flex gap-6 items-center">
            <a href="tel:+916383450508" className="hover:text-white/80 transition-colors flex items-center gap-2">
              <PhoneCall className="w-3.5 h-3.5" /> +91-6383450508
            </a>
          </div>
        </div>
      </div>
      ...
      >
        <div className="container-custom">
          <div className={`flex items-center justify-between rounded-2xl transition-all duration-500 ${!isScrolled ? "bg-white shadow-[0_20px_50px_rgb(0,0,0,0.06)] border border-border/50 px-6 py-4" : "px-2"
            }`}>
        ...
        {/* Mobile Nav Drawer */}
        <div
          className={`absolute top-full left-0 w-full bg-white/95 backdrop-blur-xl border-b border-border shadow-2xl lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "max-h-[500px] opacity-100 py-6" : "max-h-0 opacity-0 py-0"
            }`}
        >
            ...
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`px-4 py-3 text-base font-bold rounded-xl transition-colors ${isActive ? "bg-primary/5 text-primary" : "text-text-secondary hover:bg-surface-soft hover:text-primary"
                    }`}
                >
                  {link.name}
                </Link>
